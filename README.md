...
| 静态开关写入 | 走模块节点上的开关 pin —— 引擎自己的 stack UI 就是这么写的（静态参数不是 rapid-iteration 候选，必然走 override pin 分支）。外部编辑 API 永远拒绝静态输入（静态标志参与类型相等、却无法从 `UScriptStruct` 复原），所以这条不是绕行而是正路，**任何引擎都成立**。覆盖**模块自身声明**的开关；嵌套 dynamic input 上的、以及从内层模块**传播**上来的开关仍走旧路（前者需 override pin 链，后者在外层节点上没有 pin）|
