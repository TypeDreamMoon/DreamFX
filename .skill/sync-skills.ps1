...
.DESCRIPTION
    ...
    *above* where the agent is working. `.skill/` is not on that path, so the skills have to be
    ...
    tree and rewrites those two path families to whatever is correct at the destination:
        ...
        pwsh -File Plugins/DreamFX/.skill -> the real relative path to the driver
...
param(
    ...
    # Remove published dream-fx-* / reference directories that this run did not write.
    [switch]$Prune
)
...
$driverPath = Join-Path $skillRoot 'dfx.ps1'
...
$marker = '<!-- Published from Plugins/DreamFX/.skill by sync-skills.ps1. Edit the source, not this copy. -->'
...
function Get-PublishedText {
    ...
    $text = $text.Replace('Plugins/DreamFX/.skill/dfx.ps1', $driverRel)
...
$sources = @(Get-ChildItem -LiteralPath $skillRoot -Directory | Where-Object { $_.Name -like 'dream-fx-*' -or $_.Name -eq 'reference' })
if ($sources.Count -eq 0) { throw "No dream-fx-* directories under '$skillRoot'." }
...
if ($Prune -and -not $Check) {
    foreach ($stale in Get-ChildItem -LiteralPath $targetFull -Directory) {
        if ($stale.Name -notin $publishedNames -and ($stale.Name -like 'dream-fx-*' -or $stale.Name -eq 'reference')) {
